...
impl Telegram {
    ...
    pub fn read_messages(&mut self) -> Result<Vec<Message>, ureq::Error> {
        println!("{:?}", self.last_update);
        let updates: TgResult =
            ureq::get(&format!("{}getUpdates?timeout=10{}", self.base_url(), {
    ...
    pub fn send_message(&self, message: &Message) -> Result<(), ureq::Error> {
        ureq::get(
            format!(
                "{}sendMessage?chat_id={}&text={}",
                self.base_url(),
                message.chat.id,
                utf8_percent_encode(message.text.clone().unwrap().as_str(), FRAGMENT)
            )
            .as_str(),
        )
        .call()?;
